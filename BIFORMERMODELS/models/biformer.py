...
# from fairscale.nn.checkpoint import checkpoint_wrapper  # 不管



# from timm.models import register_model  # 不管
# from timm.models.layers import DropPath, to_2tuple, trunc_normal_  # 不管
# from timm.models.vision_transformer import _cfg  # 不管

from BIFORMERMODELS.ops.bra_legacy import BiLevelRoutingAttention
...
class Block(nn.Layer):
    def __init__(self, dim, drop_path=0., layer_scale_init_value=-1,
        ...
        if layer_scale_init_value > 0:
            ...
            # self.gamma1 = nn.Parameter(layer_scale_init_value * paddle.ones((dim)), requires_grad=True)
            # self.gamma2 = nn.Parameter(layer_scale_init_value * paddle.ones((dim)), requires_grad=True)
            parameter = layer_scale_init_value * paddle.ones([dim])
            self.gamma1 = paddle.create_parameter(parameter.shape, dtype=str(parameter.numpy().dtype), default_initializer=paddle.nn.initializer.Assign(parameter))
            self.gamma2 = paddle.create_parameter(parameter.shape, dtype=str(parameter.numpy().dtype), default_initializer=paddle.nn.initializer.Assign(parameter))
        else:
...
def create_model(
    ...
    kwargs = {k: v for k, v in kwargs.items() if v is not None}  # 用于过滤掉字典kwargs中值为None的键值对。
    if model_name == 'biformer_tiny':
        model = biformer_tiny(
    ...
    elif model_name == 'biformer_small':
...
def _cfg(url='', **kwargs):
    return {
        ...
        'mean': (0.5, 0.5, 0.5), 'std': (0.5, 0.5, 0.5),  # ? # (0.485, 0.456, 0.406) # (0.229, 0.224, 0.225)
